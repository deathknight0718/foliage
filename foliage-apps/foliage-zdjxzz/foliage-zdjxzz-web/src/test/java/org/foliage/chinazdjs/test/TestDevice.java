...
public class TestDevice {
    ...
    private static DataSource source() {
        ...
        source.setUrl("jdbc:h2:./target/foliage-db");
    ...
    public void testSession() throws Exception {
        ...
        Device.of("14893189398").register();
        Device.of("14811212136").register();
        {
            Device device = Device.get("14893189421");
            Assert.assertEquals(device.getName(), "device-14893189421");
            Assert.assertEquals(Device.listByPaging(0, 10).size(), 3);
            Assert.assertNotNull(device.latestGeographic());
            Assert.assertEquals(device.latestGeographic().listByProvince().size(), 2);
        }
    }
    ...
    @Test(enabled = false, dependsOnMethods = { "testRestApiRegisterDevice" })
