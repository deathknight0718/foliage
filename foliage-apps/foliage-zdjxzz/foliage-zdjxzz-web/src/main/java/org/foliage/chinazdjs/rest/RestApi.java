...
public class RestApi {
    ...
    public Response queryLatestGeographicByDeviceId(@PathParam("id") String input) {
        try {
            ...
            result.set("current", MAPPER.valueToTree(geographic));
            result.set("geographicsByProvince", MAPPER.valueToTree(geographic.listByProvince()));
            return ResponseUtils.handleSuccess(MAPPER.writeValueAsString(result));
        } catch (Exception e) {
            LOGGER.warn(Throwables.getStackTraceAsString(e));
            return ResponseUtils.handleException(e.getMessage());
        }
