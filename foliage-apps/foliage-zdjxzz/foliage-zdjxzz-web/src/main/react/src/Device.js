...
function Header() {
  ...
  return (
    <AppBar position="fixed">
      <Toolbar>
        <IconButton size="large" edge="start" color="inherit" aria-label="menu" sx={{ mr: 2 }} onClick={() => navigate(`/${REACT_APP_NAME}/`)}>
          <ArrowBackIosNew />
        </IconButton>
...
function DeviceCard(props) {
  ...
  return (
    <Card sx={{ backgroundColor: "#FAFAFA" }}>
      ...
      <CardActions>
        <Button size="small" onClick={() => navigate(`/${REACT_APP_NAME}/specification/${id}`)}>状态</Button>
        <Button size="small" onClick={() => navigate(`/${REACT_APP_NAME}/geographic/${id}`)}>位置</Button>
        <Button size="small" onClick={() => axios.delete(`api/v1/device/${id}`).then(() => onReload())}>退出</Button>
      </CardActions>
    </Card>
  );
}
...
function useEffectOfDevices(submitting, reloading, devices, onDevices) {
  const axios = useContext(AxiosContext);
  return useEffect(() => {
    if (!submitting) axios.get(`/api/v1/devices?offset=0&limit=500`).then((response) => onDevices([...response.data]));
