...
public class TestFilePoint {
    ...
    public void testWrite() {
        FilePoint bean = FilePoint.builder().withRegion("cn-pacs").withBucket("test").withPath(Identities.uuid().toString()).build();
        bean.upload(new ByteArrayInputStream("Hello Minio".getBytes()));
    }

    @Test(invocationCount = 40)
    public void testWriteWithTree() {
        FilePoint bean = FilePoint.builder().withRegion("cn-pacs").withBucket("test").withPath("test/" + Identities.uuid()).build();
        bean.upload(new ByteArrayInputStream("Hello Minio".getBytes()));
    }
    ...
    public void testObject1() {
        FilePoint bean = FilePoint.builder().withRegion("cn-pacs").withBucket("test").withPath("test").build();
    ...
    public void testObject2() {
        FileBucket bean = new FileBucket(new FileRegion("cn-pacs"), "test");
