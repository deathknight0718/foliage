 ...
 * the License.
 ******************************************************************************/
package org.foliage.jdbc.interpret.postgresql;
 ...
 * @version 1.0.0
 */
public class PostgreSqlInterpreter implements Interpreter {
    ...
    private static Logger logger = LoggerFactory.getLogger(PostgreSqlInterpreter.class);

    private static final String SQL_PATTERN_REWRITE_QUERY = "SELECT * FROM ({0}) AS PG_INTERNAL WHERE PG_INTERNAL.{1} >= ? AND PG_INTERNAL.{1} <= ?";

    private static final String SQL_PATTERN_REWRITE_RANGE = "SELECT MIN(PG_INTERNAL.{1}), MAX(PG_INTERNAL.{1}) FROM ({0}) AS PG_INTERNAL";
